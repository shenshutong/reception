...
<script>
    $(function () {
        $("#report_tab").bootstrapTable({
            url:"${pageContext.request.contextPath }/countyUser/findForecastData",
            pageNumber:1,
            pageSize:3,
            pageList:[3,6,8],
            pagination:true,
    ...
    function findDataById(id) {
        ...
        $.ajax({
            url:"${pageContext.request.contextPath}/countyUser/findDataById",
            type:"post",
            data:{id:id},
            dataType:"json",
            success:function (data) {
                if (data) {
                    alert("查询成功")
                   // $("input[name=activeName]").val(data.activeName);
                    window.location.href="/jsps/countyUser/reportData.jsp?id="+data.id;
                }else {
                    alert("失败")
                }
            },
